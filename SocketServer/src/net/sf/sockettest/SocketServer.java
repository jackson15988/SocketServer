...
public class SocketServer extends Thread {
	...
	protected static List<Socket> socketLists = new Vector<>();
	...
	private void startServer() {
		parent.setClientSocket(socket);
		InputStream is = null;
		parent.append(" [初始化] " + socket.getInetAddress().getHostAddress() + "已加入連線");
		try {
	...
	public void sendMessage(String message) {
		try {
			synchronized (socket) {
				for (Socket sc : socketLists) {
					if (out == null) {
						out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(sc.getOutputStream())), true);
					}
					out.print(message + NEW_LINE);
					out.flush();

					System.out.println("獲得的IP跟PORT : " + sc.getInetAddress() + ":" + sc.getPort());
				}
			}
		} catch (Exception e) {
			System.out.println("回傳訊息時候發生異常:" + e);
		}
